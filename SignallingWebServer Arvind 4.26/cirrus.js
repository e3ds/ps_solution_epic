...
// If you are using AWS or have to announce external public ip then enable following line and provide you Elastic ip
//var clientConfig = { type: 'config', peerConnectionOptions: {}, 'elasticip':'99.60.91.1' };
...
streamerServer.on('connection', function (ws, req) {
	...
	ws.on('message', function onStreamerMessage(msg) {
		...
		if (msg.type == 'answer') {
			...
			console.log(`<- sending webrct setting to all players}`);
...
playerServer.on('connection', function (ws, req) {
	...
	ws.on('message', function (msg) {
		...
		if (msg.type == 'offer') {
			...
			//console.log(`<- player ${playerId}: settings\n${msg.data}`);
			msg.playerId = playerId;
			streamer.send(JSON.stringify(msg));
		} else if (msg.type == 'gettings') {
			console.log(`<- player ${playerId}: gettings\n${msg.data}`);
			msg.playerId = playerId;
			streamer.send(JSON.stringify(msg));
		}
