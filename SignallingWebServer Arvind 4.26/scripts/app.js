...
/* Arvind: Make it false to remove overlay  */
...
function setupHtmlEvents() {
	...
	if (muteaudio !== null) {
		muteaudio.onchange = function (event) {
		
			if (webRtcPlayerObj) {

			   ws.send(JSON.stringify({ type: 'settings', mute:  muteaudio.checked   }));
...
function showPlayOverlay() {
	...
	setOverlay('clickableState', img, event => {
		if (webRtcPlayerObj)
			...
			webRtcPlayerObj.video.muted =false;
		requestQualityControl();
...
function WebRtcStats() {
	...
	webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
		...
		audio_receivedBytesMeasurement = 'kB';
		audio_receivedBytes = aggregatedStats.hasOwnProperty('audiobytesReceived') ? aggregatedStats.audiobytesReceived : 0;
		let audio_dataMeasurements = ['MB', 'GB'];
		audio_receivedBytes = audio_receivedBytes / 1000;
		for (let audio_index = 0; audio_index < audio_dataMeasurements.length; audio_index++) {
			if (audio_receivedBytes < 100 * 1000)
				break;
			audio_receivedBytes = audio_receivedBytes / 1000;
			audio_receivedBytesMeasurement = audio_dataMeasurements[audio_index];
		}

	
		data_receivedBytesMeasurement = 'B';
		data_receivedBytes = aggregatedStats.hasOwnProperty('databytesReceived') ? aggregatedStats.databytesReceived : 0;
		let data_dataMeasurements = ['kB', 'MB', 'GB'];
		for (let data_index = 0; data_index < data_dataMeasurements.length; data_index++) {
			if (data_receivedBytes < 100 * 1000)
				break;
			data_receivedBytes = data_receivedBytes / 1000;
			data_receivedBytesMeasurement = data_dataMeasurements[data_index];
		}


		data_sentBytesMeasurement = 'B';
		data_sentBytes = aggregatedStats.hasOwnProperty('databytesSent') ? aggregatedStats.databytesSent : 0;
		let data_sentMeasurements = ['kB', 'MB', 'GB'];
		for (let data_sindex = 0; data_sindex < data_sentMeasurements.length; data_sindex++) {
			if (data_sentBytes < 100 * 1000)
				break;
			data_sentBytes = data_sentBytes / 1000;
			data_sentBytesMeasurement = data_sentMeasurements[data_sindex];
		}
		...
		statsText += `<div>Video Received (${receivedBytesMeasurement}): ${numberFormat.format(receivedBytes)}</div>`;
		statsText += `<div>Audio Received (${audio_receivedBytesMeasurement}): ${numberFormat.format(audio_receivedBytes)}</div>`;
		statsText += `<div>Data Received (${data_receivedBytesMeasurement}): ${numberFormat.format(data_receivedBytes)}</div>`;
		statsText += `<div>Data Sent (${data_sentBytesMeasurement}): ${numberFormat.format(data_sentBytes)}</div>`;
...
function connect() {
	...
	ws.onmessage = function (event) {
		...
		if (msg.type === 'config') {
			...
			console.log("<- SS: %o", msg.sdp);
			onWebRtcOffer(msg);
		} else if (msg.type === 'gettings') {
			console.log(`message type: ${msg.type}`);
			setServerWebrtcValues(msg.data);
