...
(function (root, factory) {
    ...
    function webRtcPlayer(parOptions) {
        ...
        oniceconnectionstatechange = function(evt) {
            console.info('ice connection state change:', evt.target.iceConnectionState);
           // console.info('iceConnectionState: ', self.pcClient.iceConnectionState);
        };
...
*/
        handleCreateOffer = function (pc) {
             pc.createOffer(self.sdpConstraints).then(function (offer) {
                 ...
                 offer.sdp = offer.sdp.replace("useinbandfec=1", "useinbandfec=1; minptime=10; stereo=1; maxaveragebitrate=510000");
                 pc.setLocalDescription(offer);
                 if (self.onWebRtcOffer) {
                     // (andriy): increase start bitrate from 300 kbps to 20 mbps and max bitrate from 2.5 mbps to 100 mbps
                     // (100 mbps means we don't restrict encoder at all)
					...
					offer.sdp = offer.sdp.replace("a=extmap-allow-mixed", 'a=msid-semantic: WMS');
        ...
        handleCreateAnswer = function (pc) {
             pc.createAnswer().then(function (answer) {
                // answer.sdp = offer.sdp.replace("useinbandfec=1", "useinbandfec=1;stereo=1;maxaveragebitrate=128000");
                 pc.setLocalDescription(answer).then(function() {
  
                if (self.onWebRtcAnswer) {
                          console.log( "answer %o", answer.sdp);
                        self.onWebRtcAnswer(answer);
                    }
            ...
            return function(stats){
                ...
                stats.forEach(stat => {
                   // console.log(JSON.stringify(stat, undefined, 4));
                    ...
                    else if ((stat.mediaType == 'data-channel' || stat.id.toLowerCase().includes('data'))) {

                         newStat.databytesReceived = stat.bytesReceived;
                         newStat.databytesSent = stat.bytesSent;
                    }
      ...
      this.requestOffer = function() {
           ...
           // handleCreateOffer(self.pcClient);
            self.onWebRtcrequestOffer("offer");
        };
	  ...
	  this.receiveoffer = function(offer) {
           // console.log("offer: %o" , offer.sdp );
            var answerDesc = new RTCSessionDescription(offer);
            self.pcClient.setRemoteDescription(answerDesc).then(function() {
               handleCreateAnswer(self.pcClient);
              }) ;
